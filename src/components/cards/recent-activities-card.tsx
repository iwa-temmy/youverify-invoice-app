...
const RecentActivitiesCard = () => {
  return (
    <Card className="w-full md:w-full lg:w-[38%] h-[760px] p-4">
      <CardHeader className="flex flex-row justify-between items-center">
        ...
        <Button
          variant="outline"
          className="w-fit text-blue uppercase rounded-[30px] font-medium border border-[#E3E6EF] h-12 px-8"
        >
          View all
        </Button>
      </CardHeader>
      <CardContent className="flex flex-col gap-8 mt-4">
        {invoiceActivities?.map((item, index) => (
          <div className="flex items-center gap-6" key={index}>
            ...
            />
            <div>
              <h4 className="text-lg font-medium text-neutralBlack">{item.action}</h4>
